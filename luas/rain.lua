...
local r_rainradius = cvar.r_rainradius
local r_rainalpha = cvar.r_rainalpha
local r_rainwidth = cvar.r_rainwidth
...
local r_rainradius_slider = ui.new_slider("VISUALS", "Effects", "r_rainradius", 0, 1500, 1000)
local r_rainalpha_slider = ui.new_slider("VISUALS", "Effects", "r_rainalpha", 0, 100, 8, true, nil, .01)
local r_rainwidth_slider = ui.new_slider("VISUALS", "Effects", "r_rainwidth", 0, 100, 8, true, nil, .01)
    ...
    pre_render = function(self)
        if ui.get(checkbox) and entity.get_local_player() then
            ...
            if not self.precipitation_client_class then
                ...
                while(cur_class) do
                    if cur_class.class_id ==  138 then
...
local signature = client.find_signature("vstdlib.dll", "\x55\x8B\xEC\x8B\x0D\xCC\xCC\xCC\xCC\xFF\x75\x08") or error("failed to find sig")
local RandomSeed = ffi.cast("int(__cdecl*)(int)", signature) -- the aimbots hitchance calls RandomSeed so the rain looks like shit if i dont have this autism

client.set_event_callback("paint", function()
	r_rainradius:set_int(ui.get(r_rainradius_slider))
	r_rainalpha:set_float(ui.get(r_rainalpha_slider) / 100)
	r_rainwidth:set_float(ui.get(r_rainwidth_slider) / 100)
	RandomSeed(math.random() * 10000000) 
end)
